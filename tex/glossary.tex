...
\newacronym[
  description={
    The \gls{matrix engine} extension for the x86 \gls{isa}.
  }
...
\newacronym[
  description={
    ...
    It has since developed into a standard interface for higher (matrix-matrix) and lower level (scalar/vector, vector/vector) linear algebra operations with many open source and proprietary implementations.
  }
...
\longnewglossaryentry{broadcast}{name={Broadcast}, text={broadcast}}{
  The act of duplicating a scalar value to all \glspl{lane} in a vector register.
}
...
\longnewglossaryentry{dead}{name={Dead},text={dead}}{
  A value in a program is dead if it is guaranteed that the value will never be used again.
  A static liveness analysis is performed to determine which values are dead at a given point in a program.
  Therefore, a value that is dead at a given point of the program must be dead for all possible executions of the program.
  A variable may be dead or a register may be dead.
}
% F
\newacronym[
  description={
...
\newacronym[
  description={
    ...
    \gls{simd} equivalents exist, performing the same operation per \gls{lane} of the operand vectors.
  }
...
\longnewglossaryentry{lane}{name={Lane},text={lane}}{
  A way to refer to an element of a vector register or operation.
  Derived from the way circuit pathways leave and enter vector functional units in groups corresponding to an element, like the lanes of a roadway.
...
\longnewglossaryentry{live}{name={Live},text={live}}{
  A value is live at a \gls{point} in a program if there exists at least one path from this \gls{point} that may use the value.
}
...
\longnewglossaryentry{matrix engine}{name=Matrix Engine, text={matrix engine}}{
  A general term describing a set of new facilities on the most recent generation of \glspl{cpu} which focus on matrix operations.
}
\newacronym[
  description={
...
\longnewglossaryentry{object file}{name=Object File,text={object file}}{
  ...
  Compilers may skip the object file and directly generate an executable if a program consists of a single \gls{translation unit}.
}
...
\longnewglossaryentry{spill}{name={Spill},text={spill}}{
  The act of storing a \gls{live} register to memory temporarily in order to free the register for another value.
}
\newacronym[
  description={
...
\newacronym[
  description={
    The 64 128-bit registers defined as part of \gls{vsx}.
    They can be interpreted as vectors of two 64-bit, four 32-bit, eight 16-bit, or 16 8-bit elements.
  }
]{vsr}{VSR}{Vector-Scalar Register}
