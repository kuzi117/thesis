...
The loop on line~\ref{unrollLoopStart} will unroll the loop as many times as is necessary to perform all accumulations.
Thus the number of loads and outer products performed will change automatically as $K$ increases or decreases.
The only restriction to consider is that when smaller data types are used and the rank $r$ of the update does not perfectly divide $K$, the final accumulation will not have enough data to fill the argument registers.
