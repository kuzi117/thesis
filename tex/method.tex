...
The original implementation also includes one other significant constraint: the \gls{intrinsic} must be called with the input matrices already loaded from memory into a virtual register and does not use pointers.
...
When the \gls{intrinsic} is executed each accumulator computes $K$ outer products using a \gls{mma} outer-product instruction.
...
Until now, the algorithms have used values of $M$, $N$, and $K$ selected such that a micro kernel with the accumulator arrangement shown in \rfig{intrinsic} could be computed with a single set of assemble and disassemble instructions.
However, the implementation of \ralg{intrinsic} in LLVM must handle any \code{llvm.matrix.multiply.*} \gls{intrinsic} created by any compilation path and thus must handle arbitrary values for $M$, $N$ and $K$.
...
Arbitrary values for $K$ are implicitly handled in \ralg{intrinsic}.
