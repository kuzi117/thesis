...
\section{Summary}
This section presents a compiler-only method for the code generation of a high-performance matrix-multiplication kernel.
It begins by introducing \gls{llvm} intrinsics and their format with a specific focus on the \code{llvm.matrix.multiply} intrinsic.
This particular intrinsic serves as the starting point for the generation of the matrix multiplication kernel.

As an example, the intrinsic's preexisting vectorisation lowering serves to introduce the usage and constraints of the intrinsic.
The remaining sections of the chapter discuss how the intrinsic can be made to produce code targetting a \gls{matrix engine}, specifically \gls{mma}.
The examination of an $8 \times 16$ \code{float}-element kernel serves as the computation's base case, demonstrating how arguments are extracted from input matrices and how accumulators can be used to efficiently produce the computation result.
...
First, an method to extend the presented algorithm to each of the types natively supported by \gls{mma}.
