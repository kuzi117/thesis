...
The evaluation was performed on a first-silicon version of \gls{power10} and, as such, the frequency may be different than what will be featured in commercially available versions.
The results presented in this chapter are expected to be representative of the hardware that will eventually be available to consumers but the performance of the commercial machines may differ.
...
Every measurement presented in this chapter was produced using the Google benchmark library~\autocite{googlebench}.\footnotemark
...
Efficient \glslink{spill}{spilling} requires careful scheduling but in the case of \gls{mma} the problem is made more difficult by the following issues.
...
This \glslink{spill}{spilling} issue must be resolved by the backend scheduler: the scheduler needs to move loads closer to the \gls{point} in the program that contains the first use of the loaded value to reduce register pressure by using a technique called ``load sinking.''\footnotemark
...
A workaround for the \glslink{spill}{spilling} that results from unsunk loads, which can be used until the backend scheduler improves, consists of modifying the \gls{ir} by further breaking up the vector loads into smaller vector loads and sinking these smaller loads closer to the first use of the loaded values.
...
When lowered by the backend, this single contiguous load is automatically broken into register-sized pieces that are then, importantly, also \glslink{spill}{spilled} in the same format.
...
The figure shows an example large kernel ($16 \times 32$) in the style of \rfig{intrinsic}; to illustrate how \glspl{spill} occur, the figure does not divide individual elements nor does it show $B$.
...
\begin{figure}[t]
 ...
 %   \draw[pattern={Dots[angle=45,distance={7pt},xshift=.1pt, radius=1.25]}, shift={(11, 6)}] (0, 0) rectangle +(8, 2);
    ...
    \draw[decorate, decoration={brace, mirror}] (0, -0.3) -- node[below=2] {$D$} +(2, 0);
  \end{tikzpicture}
  \caption[Large Kernel Operand Reuse]{Demonstration of operand reuse in large kernels.}
  \label{fig:opReuse}
\end{figure}

According to constraint \fbox{5} in \rsec{baseCase}, \glslink{spill}{spilling} an accumulator is expensive, and therefore an efficient solution must finish computing a block of accumulators before moving on to a different area of the matrix.
Doing so requires that every operand along the dimension $D$ be brought to a register before any operand can be reused with a new block of accumulators.
...
Using only the 32 registers available while the accumulators are assembled, if $D \geq \left\lceil \frac{32}{6} \right\rceil = 6$, then there is not enough space for all values to remain in registers.
...
For smaller data types, however, \glslink{spill}{spilling} is the correct choice.
Materialising an \code{i16} involves two loads and at least one instruction to shuffle the loaded data together.
Reproducing the value after spilling becomes a simple load, significantly reducing register pressure and required memory bandwidth.
Both the original version and the load-sinking version correctly choose to \gls{spill} their values instead of \glslink{rematerialisation}{rematerialising} them.
...
The original version loads 16 elements at a time using 256-bit vector-pair loads, meaning that the load-sinking version quadruples the number of loads required to load the values needed for the computation (excluding loads from the original \glslink{spill}{spilling} issue).
Changing the scheduling to remove the unnecessary loads and stores resulting from \glslink{spill}{spilling} would likely lead to a much more significant performance gap between the two versions.
...
Unlike issues with \glslink{spill}{spilling}, it is likely that performance degradations caused by misbehaving shuffles can be solved by creating type-specific shuffling code.
...
Such casting should produce no new instructions.
...
This is combination transformation is only possible if access orders match the expected access orders for larger types, \ie column major for $A$ and row major for $B$.
...
Transposing the output so that it can be stored into a column-major output-matrix requires extra shuffles.
...
The implemented framework does not allow for irregularly shaped layouts.
...
In this case, the rectangular layouts should be more performant because they require less operands per accumulation (six instead of eight) and therefore allow more accumulations to be setup simultaneously.
If it is not the case that a group of loads followed by outer products is the most performant option, and instead interleaving loads with outer products is more performant, then either set of layouts can potentially be more performant, as long as the latency for the larger number of loads required to setup the linear layout can be effectively hidden.
...
{\interfootnotelinepenalty=10000\footnotetext{The load sinking assemblies also contain the more efficient paired-vector loads though it is likely due to a code-transformation pass ordering issue, not deliberate choice.}}
...
Following the results in \rfig{floatSink}, fixing the \glslink{spill}{spilling} issue for the \code{float} rectangular layouts should quarter the number of cycles required.
...
\footnotetext{
  ...
  Choosing $2 \times 4$ also ensures that all types are equally disadvantaged by \glslink{spill}{spilling}.
}
...
However, both kernels are equally disadvantaged when it comes to the initial \glslink{spill}{spilling} issue, both are subject to the same kernel unrolling scheme (\ie completely unrolled), and both use the same register-allocation policy.
...
\rfig{mmaVsx} and \rtab{mmaVsx} show the effect of \gls{mma} on the \gls{lowering} of the \code{llvm.matrix.multiply.*} \gls{intrinsic}.
...
The results presented by Moreira \etal are likely at, or near, the theoretical peak performance for \gls{mma} and these results do not match them due, in large part, to the \glslink{spill}{spilling} issue.
...
The final point of comparison is the handwritten single precision floating point kernel from Moreira \etal.
...
This finding points to a potential direction for improvement after the \glslink{spill}{spilling} issues are solved.
...
This observation points to a potential great achievement by the code-generation algorithm because it implies that, once the \glslink{spill}{spilling} issues are resolved, the code generated by the compiler will be on-par with handwritten code.
...
Following the experimental setup is a discussion of two important issues that must be addressed in future versions of the assembly-code generator in the backend: \glslink{spill}{spilling} and shuffling.
