...
\begin{table}[t]
  ...
  \caption{
    ...
    \bk{I could combine items vertically when they're repeated but it's hard to tell where they stop/start without horizontal lines.}
  }
  \label{tab:floatSink}
\end{table}
...
For \code{float}, \code{i16}, and \code{i8}, the numbers are inline with the hypothesised performance (see below for discussion of \code{half}).
The two order-locked variants ($\textrm{C} = \textrm{C} \times \textrm{C}$ and $\textrm{R} = \textrm{R} \times \textrm{R}$) perform worse than the two with optimal layouts ($\textrm{C} = \textrm{C} \times \textrm{R}$ and $\textrm{R} = \textrm{C} \times \textrm{R}$).
\nelson{Be specific about which two kernels are laid out optimally.}
\bk{Does specifying it in the previous sentence count now?}
...
In each case, either $A$ or $B$ is laid out ideally while the other is not.
\nelson{
  Refer to the bars in the plots when making these observations.
  For instance, is the next sentence about the difference between the two last column of half?
  For the other types there is not difference between C x R and R x C.
}
\bk{
  The beginning of this paragraph starts by saying that we're discussing the two order-locked variants.
  We're therefore talking about the first two columns in float, i16, and i8.
  I want to ask about where the misunderstanding comes from before adding column labels because I think they're labeled ok.
}
...
\rsec{baseCase} describes layouts for the eight accumulators available in \gls{power10} \gls{mma} --- $2 \times 4$ and its transpose $4 \times 2$ --- that have the highest factor of reuse, with operands from one matrix being reused four times and an operand from the other matrix being used twice.
...
With 32 registers available, using only six of these registers per accumulation allows for the operands needed for future accumulations to be brought to registers ahead of time or even for multiple sets of accumulations to be performed uninterrupted.
\nelson{What do you mean by "for multiple accumulations to be performed uninterrupted"? Are there cases where the accumulations are interrupted?}
\bk{Added ``sets of''; is that acceptable?}
...
\nelson{
  ...
  Are you saying that 2x4 and 4x2 have identical performance, and that 1x8 and 8x1 have identical performance?
  Or are you saying that they all have identical performance?
  Which is not true according to the graph.
}
\bk{
  I mean to say that 2x4 and 4x2 have the same performance and that 1x8 and 8x1 have the same performance.
  I recognise that the 1x8/8x1 have a slight difference in performance (as I mentioned earlier in my comment after the next sentence) but I'm unsure why.
  The only thing I can come up with is bad code generation by the backend but really determining that is likely to require using the simulator and implementing traces in the framework which is unlikely to be a short process.
  I can reexamine the assembly but it's going to be pretty difficult to determine that way.
}
...
\bk{
  ...
  For \code{i16}, the $8 \times 1$ takes longer than its transpose and the rectangular layouts are identical.
...
\nelson{
  What "deconflict \gls{live} values" mean?
  Are you talking about reducing register interference?
}
\bk{
  Yes, that's exactly what I'm talking about.
  I didn't think think it was prudent to begin an in-depth discussion of live ranges and register interference so I tried to step around it while still bringing the essence of the argument.
  If you think I should bring more of that discussion I can add it.
}
...
\bk{
  In this case, it's the kernel used by Jose Moreira, which is better and worse in some ways than BLAS.
  This is only really the case for \code{float}.
  Jose Moreira has told me he only has DGEMM and SGEMM kernels and it's likely that BLAS-like libraries haven't bothered with anything else either.
  I'm not sure about Eigen, but I don't think it has it set up either.
  For any of the smaller types, it's likely the kernel doesn't exist except maybe in a hardware verification test.
  That being said, I know what the kernel should look like if I had to write it by hand.
}
