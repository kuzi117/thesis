...
\rsec{registerArguments} shows that \gls{mma} is sensitive to the access order of elements in \glspl{vsr} and, therefore, also sensitive to the access order of the matrices in memory from where values are loaded into registers.
...
For \code{float}, when stored \gls{vsr} a row-major matrix is indistinguishable from a single column of four elements.
...
\rfig{accessOrder} shows the effect of matrix access order on performance.
...
\nelson{Be specific about which two kernels are laid out optimally.}
...
Based on the discussion in \rsec{orderExpectation}, it is also unsurprising that $\textrm{R} = \textrm{R} \times \textrm{R}$ outperforms $\textrm{C} = \textrm{C} \times \textrm{C}$.
...
\nelson{Refer to the bars in the plots when making these observations. For instance, is the next sentence about the difference between the two last column of half? For the other types there is not difference between C x R and R x C.}
...
\rsec{baseCase} describes layouts for the eight accumulators available in \gls{power10} \gls{mma} --- $2 \times 4$ and its transpose $4 \times 2$ --- that have the highest factor of reuse, with operands from one matrix being reused four times and an operand for the other matrix being used twice.
...
\nelson{What do you mean by "for multiple accumulations to be performed uninterrupted"? Are there cases where the accumulations are interrupted?} accumulations to be brought to registers ahead of time or even for multiple accumulations to be performed uninterrupted.
...
\rfig{tightAccLayout} shows the effect of accumulator layout on performance.
\nelson{This sentence is not clear. Are you saying that 2x4 and 4x2 have identical performance, and that 1x8 and 8x1 have identical performance? Or are you saying that they all have identical performance? Which is not true according to the graph.}
...
An initial analysis of this difference may hypothesize that, for \code{float}, the $1 \times 8$ layout and its transpose had found a better load schedule or that it had activated hardware such as the stream prefetcher in a way that was not possible with the data types that loaded less data overall.
It is true that the code generator found a better schedule, though not in the way that we anticipated.
Instead, the changed arguments have simplified the backend code generator's analysis, likely its \glslink{live}{liveness} analysis, 
\nelson{What "deconflict \gls{live} values" mean? Are you talking about reducing register interference?} 
...
\nelson{Where this handwritten assembly that you are comparing with comes from? Was it introduced in this document before?}
...
\nelson{Simply state the reason with a few words instead of saying "for the same reasons". The reference is to this same section.}
As explained in \rsec{layoutAnalysis}, the linear layouts were significantly slower for the same reasons discussed and so, for the sake of brevity, will not be reiterated.
\bk{Is this acceptable or should I reiterate.}
...
\nelson{same comments as for the previous table in terms of the precision that you use to report the values. Make the table fit within the margins.}
\subsection{Analysis}
...
While the vectorised lowering of \code{llvm.matrix.multiply} is the most comparable point of reference when it comes to improving upon compiler-only methodologies for matrix multiplication, another important point to study is overall achievable performance.
...
The first two are the intrinsic lowering for \code{float}, one with no transformations applied and the second with the load-sinking transformation applied.
...
In all cases, the kernels use \code{float} elements while computing a \matmul{8}{32}{16} kernel.
...
\begin{table}[t]
  ...
  \makebox[\textwidth][c]{
    \begin{tabular}{| c | c | c | c |}
      \hline
      Name & $n$ & CPU Time (\SI{}{\textit{\nano\second}}) & Cycles\\\hline
      No Load Sinking & $17995677.20 \pm 298626.48$ & $77.92 \pm 1.31$ & $311.31 \pm 5.21$ \\\hline
      Load Sinking & $35076889.96 \pm 42722.35$ & $39.89 \pm 0.01$ & $159.36 \pm 0.05$ \\\hline
      Handwritten & $31504866.20 \pm 51489.54$ & $41.15 \pm 0.07$ & $164.39 \pm 0.27$ \\\hline
    \end{tabular}
  }
  \caption{
...
While the slight advantage in \rtab{intrHand} remains a significant and promising result, given the near identicalness of the assemblies, it is difficult to conclude that one truly outmatches the other.
