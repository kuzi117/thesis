...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=1]
    \draw[decorate, decoration={brace}, align=center, shift={(0, 0)}] (0, 0) -- node[above=2] {$n$ kernels} +(2, 0);
    \draw[decorate, decoration={brace}, align=center, shift={(2.125, 0)}] (0, 0) -- node[above=2] {$n$ kernels} +(2, 0);
    \node[shift={(4.5, 0)}] {$\ldots$};
    \draw[decorate, decoration={brace}, align=center, shift={(4.875, 0)}] (0, 0) -- node[above=2] {$n$ kernels} +(2, 0);
    \draw[decorate, decoration={brace}, align=center, shift={(1, .8)}] (0, 0) -- node[above=2] {10 repeats} +(4.875, 0);
...
Every result presented in this chapter was produced using the Google\texttrademark{} benchmark library\footnotemark~\autocite{googlebench}.
\footnotetext{At commit ab74ae5e104f72fa957c1712707a06a781a974a6.}
The framework implements a method of timing a kernel by placing it in a loop whose exit condition is dynamically determined by the benchmark being executed.
The loop will only exit when the results collected have been executed enough times to obtain statistical stability.

To achieve this, an execution count, $n$, is determined by
\begin{enumerate*}[itemjoin={{; }}, itemjoin*={{; and }}, label=(\arabic*), after={\footnotemark.}]
  \item timing several executions of the benchmark
  \item multiplying the required time (set at one second\footnotemark) by $1.4$
  \footnotetext{One second is double the default, suggested half-second time-limit.}
...
In \rfig{execution}, completing $n$ iterations of the loop corresponds to a single of the lower braces labeled ``$n$ kernels''.
This timing process is then repeated another nine times, the upper brace in \rfig{execution}, to reduce the impact of system fluctuations in any execution.
Each repetition uses exactly the same number of executions; $n$ is not recalculated.
