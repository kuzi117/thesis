...
Every iteration of the loop, the function \code{RunNIterations} is called.
\code{RunNIterations} executes and times the tested kernel \code{iters} times and returns statistics.
These results are passed to the function \code{ShouldReportResults} that decides if the results of the latest set of kernel executions have reached stability.
A simple metric determines this condition: did the kernel execute more than a maximum iteration count (one billion iterations) or was the total time spent executing on the \gls{cpu} (\code{totalCpuTime}) longer than a threshold (\code{minTime}, one second).
...
The presented framework thus aims to overestimate the required iterations to ensure that execution time meets or exceeds \code{minTime}.

To implement this strategy, \code{PredictItersNeeded} first creates a multipler that, if the kernel execution time is very consistent, would perfectly scale the total execution time to be $1.4 \times \textrm{\code{minTime}}$.
...
If it was a significant portion, then the multiplier is unchanged, with the assumption that $\textrm{\code{iters}} \times \textrm{\code{multiplier}}$ iterations is enough for the total exection time to be greater than \code{minTime}.
Otherwise, if the total time was not significant, the multiplier becomes 10.
...
Thus, because the execution time of the kernels presented in this thesis are so consistent, a reasonable summary is as follows:
...
Each measurement produces two values alongside the iteration count: the mean \gls{cpu} execution time and the mean cycles passed in the kernel.
Vertical bars in plots represent mean cycles and the attached error bars represent the 95\% confidence interval across the 25 repetitions.
Tables accompany each plot with mean iteration count ($n$), mean \gls{cpu} time, mean cycles, and their respective confidence intervals.
