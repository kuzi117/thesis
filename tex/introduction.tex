...
An external accelerator may be an \gls{asic} specialised to the task (\eg Google's \gls{tpu}~\autocite{abadi2016tensorflow}), a \gls{gpu}, or \ifglsused{fpga}{an}{a} \gls{fpga}.
...
However, because the kernel is written at a higher level within the compiler it counters many of the issues associated with the hand-written assembly version.
