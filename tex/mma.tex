...
\rfig{innerProduct} demonstrates the inner product: an element-wise multiplication between a row of matrix $A$ and a column of matrix $B$ followed by the summation of the products (\ie the dot product).
In short, two vectors produce a single, fully-computed output element.
The outer product, shown in \rfig{outerProduct}, shows how a column of matrix $A$ and a row of matrix $B$ produce a \emph{partial} output matrix.
...
Tiling, as discussed in \todo{reference mat mul section}, breaks the operation in to smaller, more mangeable operations.
...
The opposing operation to assembly is disassembly (\code{xxmfacc}, Move From Acc).
Disassembly moves each row from the accumulator into the same vector registers used for construction.
Therefore, as before, the first row of \code{ACC[0]} will be placed into \code{VSR[0]}, the second row is placed into \code{VSR[1]}, \etc.
