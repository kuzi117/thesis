...
\begin{figure}[t]
  ...
  \end{subfigure}
  \hfill
  \caption{Element ordering in types smaller than \code{float} demonstrated using \code{i16}.}
...
Moreover, a total of seven types are supported, several of which perform mulitiple outer products in a single instruction, increasing efficiency
Finally, \gls{mma} is compared with the \gls{matrix engine} available in other \glspl{isa}.
