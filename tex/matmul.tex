...
As long as each element of $A$ is multiplied with the correct element of $B$ and accumulated to the correct element of $C$, the order in which these operations happen does not matter.
...
First, the classic method taught in linear algebra courses for computing matrix multiplication, the \emph{inner product}.
As well, with a different organisation, the \emph{outer product}.
...
As taught in the classroom, the inner product can be viewed as the repeated application of the \emph{dot product} to produce a matrix.
The dot product takes two vectors and produces a single scalar value.
...
Within the context of the inner product, the dot product is used to compute a single cell of $C$ from a row of $A$ and a column of $B$.
More concretely, given the computation \matmul{M}{D}{N}, the inner product computes the cell $c_{i,j}$ by taking the dot product of the $i$th row of $A$ and the $j$th column of $B$ (\rfig{innerProduct}).
...
\begin{lstlisting}[caption={[Basic inner product]A basic matrix multiplication via inner product.},label=lst:basicInner,language=C++,columns=flexible,morekeywords=uint64_t,escapechar=|]
...
In order to implement the mathematical process ouline above, the loops on lines~\ref{innerILoop} and \ref{innerJLoop} of \rlst{basicInner} simultaneously select the cell from $C$, the row of $A$, and the column of $B$ that will be used.
The innermost loop on line~\ref{innerKLoop} implements the dot product by multiplying elements from the chosen vectors in $A$ and $B$ and then reducing them into the chosen cell of $C$.
One can thus view the implementation in \rlst{basicInner} as a loop over $i$ and $j$ that selects an output cell and repeatedly calls a dot product operation.
...
\begin{lstlisting}[caption={[Pseudocode for inner product]A basic matrix multiplication via inner product broken into logical concepts.},label=lst:basicInnerPseudo,columns=flexible]
for i=0 to M, j=0 to N do
  C[i, j] = dotProduct(A.row(i), B.column(j))
\end{lstlisting}
...
Because the dimension $D$ dictates both the number of columns in $A$ and the number of rows in $B$, the full matrix multiplication can be computed by performing $D$ outer products and summing the resulting partial products.

Using \rlst{basicMatMul} as a model, this process implements matrix multiplication with a loop order of $(k, i, j)$:
\begin{lstlisting}[caption={[Basic outer product]A basic matrix multiplication via outer product.},label=lst:basicOuter,language=C++,columns=flexible,morekeywords=uint64_t,escapechar=|]
for (uint64_t k = 0; k < D; ++k)|\label{outerKLoop}|
  for (uint64_t i = 0; i < M; ++i)|\label{outerILoop}|
    for (uint64_t j = 0; j < N; ++j)|\label{outerJLoop}|
        C[i][j] += A[i][k] * B[k][j];
\end{lstlisting}
Here, the loop on line~\ref{outerKLoop} selects a column from $A$ and row from $B$ while the loops on lines~\ref{outerILoop} and \ref{outerJLoop} compute the outer product of those vectors.
...
\begin{lstlisting}[caption={[Basic outer product]A basic matrix multiplication via outer product broken into logical concepts.},label=lst:basicOuterPseudo,columns=flexible]
for k=0 to D do
...
The loop simply performs $D$ rank-one updates, implying the algorithm can be once again reinterpreted in the form of one rank-$D$ update like so:
...
\nelson{I do not know what a "vector-like panel" or a "cell-like block" is.}
...
\bk{
  I'm not sure this section fits, be it after the ending of the last section or anywhere in the paper.
  I've considered cutting it but wanted feedback first.
}
