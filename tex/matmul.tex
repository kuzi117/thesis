...
A given order for the loops leads to the \emph{inner product}, which is the classic method taught in linear algebra courses for computing matrix multiplication, while a different order leads to the \emph{outer product}.
...
\nelson{In this section, it seems that you are talking about the inner product of two matrices (you say that the inner product is the repeated application of the dot product). Is that what you mean?}
...
\nelson{Please add a reference to the end of this sentence so that the reader can read more if she wishes.}
