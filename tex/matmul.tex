...
\label{cha:matmul}
\nelson{I am rewording the opening sentence so that it is not too similar to the opening sentence in the introduction.}
...
\bk{
  ...
  I think ``listings'' are meant to be formatted output, e.g. printed output or resulting IR.
...
\nelson{I still find this paragraph structure difficult to follow and I think that it can be improved. Think about the message that you want to convey and try to structure it in a way that is easier to follow.}
...
When computing matrix multiplication, both the outer product and inner product compute the same output in the end but, as \rsec{outerKernel} and \rsec{innerKernel} will discuss, the efficacy with which they arrive at the destination can vary greatly.
...
\nelson{This paragraph sounds hand wavy and I am really confused about the distinction that you are trying to make between the inner product and the dot product.  https://math.stackexchange.com/questions/476738/difference-between-dot-product-and-inner-product Thus, in the case of matrix multiplication, where we are dealing with real numbers, the dot product and the inner product are the same. It just so happen that the people implementing matrix multiplication in processors and libraries decided to call it the inner product, specially when they needed to differentiate it from the outer-product implementation. I do not think that you need to even mention the dot product in your thesis. But if you decide to do so, you need to be precise in relation to the mathematical definitions because it is an archival document.}
Within the context of the inner product, the dot product uses a row of $A$ and a column of $B$ to produce a single cell of $C$.
Therefore, given the computation \matmul{M}{D}{N}, the inner product computes the cell $c_{i,j}$ by taking the dot product of the $i$th row of $A$ and the $j$th column of $B$ (\rfig{innerProduct}).
