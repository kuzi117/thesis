...
\nelson{
  I think that this section could benefit from a short opening paragraph where you sign post that you will first discuss the traditional inner-product view of the implementation of the inner kernel and will then present the more suitable outer-product implementation strategy.
}
\bk{
  I've added this paragraph, though I think it sounds a bit strange since no other section has this ``landing pad'' paragraph.
  I've added section breaks for the different pieces that the new paragraph references with the hope that it makes it more reasonable to have an exposition paragraph.
  If the paragraph needs modification or the subsections should go, let me know.
}

\bk{
  I just want to clear up that they don't view this as inner product since that would be implemented in terms of the dot product which they definitely avoid.
  Instead, they just think of it in terms of \code{gepb} or \code{gebp} (block * panel or vice versa), each of which is conveniently a small outer-product though they never acknowledge it.
  It makes for a convenient link to MMA though.
}

This section carries forward the ideas discussed in \rsec{outerKernel}, linking the blocking strategy in the outer kernel with that of the inner kernel.
Discussed next is the typical library's implementation of these concepts using \gls{simd} techniques in a simple but flawed outer-product.
Following this is a presentation of a method of resolving these flaws through an improved version of the outer product.

\subsubsection{Connections to the Outer Kernel}
\begin{figure}[t]
  \hfill
  \begin{subfigure}{.45\linewidth}
    \centering
...
\subsubsection{Implementation in Libraries}
...
While viewing this operation as a traditional inner-product is tempting, a different point of view can be more beneficial.
Instead, we can choose to recognize the multiplication as a degenerate case of an outer product where one operand vector's length is one.
...
Given a register that could contain the much larger output data in the blue boxes, the overhead of broadcasting can be completely removed and the computational density of the operation quadrupled.
\bk{
  Is ``computational density'' a known idea?
  We used it in our paper for PACT.
  Would it be better to say that output data is quadrupled or some other way of stating that idea?
}
