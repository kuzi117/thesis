...
In a hypothetical machine with unlimited resources, including vector registers of unlimited length, they would performed the same number of stores and loads.
...
Tiling, as discussed in \rsec{blockPack}, breaks the whole-matrix multiplication into smaller, more manageable computations.
As discussed in that same section, on the macro scale, Goto and van de Geijn conclude that a dot-product-based tiling results in more costly data movement, making it a poor choice for the implementation of the inner kernel.
...
\nelson{It is not conventional to use the notation A x B = C. The conventional notation is C = A x B.}
...
Instead, this multiplication can be viewed as a degenerate case of an outer product where one of the operand vector's length is one.
...
\nelson{Does computational throughput capture what you mean by computational density?}
