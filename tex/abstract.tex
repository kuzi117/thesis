...
A less common, yet critical, opportunity for code tranformation which depends on inlining is operation combination via loop fusion.
...
The most recent processor in the \gls{power}\textsuperscript{\textregistered{}} architecture, \gls{power10}, from \gls{ibm}\textsuperscript{\textregistered{}} features one such extension named \gls{mma}.
...
This thesis also features a detailed analysis of the experimental results --- especially for some data types for which the performance is surprising or lower than expected --- that reveals opportunities for changes in the compiler that have the potential to lead to improvements in the entire \gls{power} compilation stack.
